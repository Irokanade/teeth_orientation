...
def checkFlag3D(oriPltImage,resizeScale,proportion, imageWidth, imageHeight):
...
def getFace():
    ...
    # Test all image
    # path setting
    # load every file in ./dataset/rotated
    root = 'dataset'
    samdir = 'rotated'
    ...
    with open('specialLog.log','w') as specialLogFile :
        for fileName,imgSet in imgs.items():
            ...
            for imageName in imgSet:
                ...
                if checkFlag3D(pltImage,resizeScale,whiteProportion, imageWidth, imageHeight) == False:
                    ...
                    for i in range( len(imageInfoSet) ):
                        if imageInfoSet[i].useFlag == False:
                            ...
                            if leftAverage > rightAverage:
                                ...
                                rightViewCnt += 1
                                csvWriter.writerow([info.imageName,"Right"])
                checkClassication(leftViewCnt,rightViewCnt,imageInfoSet, specialLogFile)
